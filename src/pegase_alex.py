...
def _extracted_from_traitement(tabminutes, pause):
    ...
    if soir == HMIN:
        depart = _conversion_heures(soir) + " (" + _compute_gain(arrivee, pause, soir) + ")"
    else:
...
def _compute_gain(arrivee: int, pause: int, depart: int) -> str:
    ...
    gain = (DUREE_JOUR - duree) if duree < 600 else (DUREE_JOUR - 600)

    if gain < 0:
        result = "gain" if duree < 600 else "gain (max)"
        gain *= -1
    else:
        result = "perte"
